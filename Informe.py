# -*- coding: utf-8 -*-
"""
Created on Tue Jun 30 20:37:48 2026
...
# Capturar las columnas de meses en su orden original de Excel (Texto puro)
...
# Clientes Nuevos: Buscamos el primer mes cronológico en que compró cada uno
...
# E. UNIÓN FINAL EN TABLA MAESTRA (Todos los 'Mes' son tipo Texto/String aquí)
...
app = dash.Dash(
    __name__, 
    external_stylesheets=[dbc.themes.BOOTSTRAP, "https://googleapis.com"],
...
app.layout = dbc.Container([
    # Encabezado
    dbc.Row([
        dbc.Col(html.H2("Dashboard de Control Comercial (Feb 2025 - Jun 2026)", className="fw-bold my-4 text-dark"), width=12)
    ]),
    ...
    dbc.Row([
        ...
        dbc.Col(dbc.Card([
            html.H6("PROMEDIO CLIENTES / MES", className="text-muted small fw-bold"),
            html.H3(f"{int(df_final['Clientes_Mensuales'].mean())}", className="text-success fw-bold m-0")
        ], style=CARD_STYLE), width=3),
    ...
    dbc.Row([
        dbc.Col(dbc.Card([
            ...
            dcc.Graph(id='grafico-comercial')
        ], style=CARD_STYLE), width=6),
        
        dbc.Col(dbc.Card([
            html.H5("Análisis de Dinámica de Clientes", className="fw-bold text-secondary mb-3"),
            dcc.Graph(id='grafico-comportamiento-clientes')
        ], style=CARD_STYLE), width=6),
    ], className="g-3 mb-4"),
...
@app.callback(
    [Output('grafico-comercial', 'figure'),
     Output('grafico-comportamiento-clientes', 'figure')],
    [Input('grafico-comercial', 'id')]
)
def renderizar_graficas(_):
    # Gráfico 1: Ventas y Maquilas
    ...
    # Gráfico 2: Clientes Activos vs Nuevos
    fig_clientes = px.bar(
        df_final, x='Mes', y=['Clientes_Mensuales', 'Clientes_Nuevos'],
        barmode='group',
        labels={'value': 'Cantidad de Clientes', 'variable': 'Segmento'},
        template="plotly_white",
        color_discrete_sequence=["#198754", "#0dcaf0"]
    )
